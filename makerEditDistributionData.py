...
class Controller(makerController.SuperController):
    def drawDialog(self):
        ...
        self.dlg.defaultFolderButton.Bind(
            self.view.wx.EVT_BUTTON, self.editDefaultRemoteFolders
        )
            ...
            for newDataSet in newData:
                if newDataSet["ftp_source"] == fileToCompare:
                    if newDataSet["remote_dir"] == theOldDataSet["remote_dir"]:
                        ...
                        toCleanup.append(
                            (
                                (theOldDataSet["remote_dir"], theOldDataSet["target"]),
                                (newDataSet["remote_dir"], newDataSet["target"]),
                            )
                        )
    ...
    def cleanup(self, toCleanup):
        ...
        for x in toCleanup:
            ...
            if self.project.checkIfRemoteDirIsDir(finalTargetFolder):
                self.project.renameRemoteFile(
                    urlparse.urljoin(finalSourceFolder, source[1]),
                    urlparse.urljoin(finalTargetFolder, target[1]),
                )
            else:
