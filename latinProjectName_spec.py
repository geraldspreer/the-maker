...
class ProjectManagerTestController(makerProjectManager.ProjectManagerController):
    ...
    def loadEditorStylesAndCreateMenu(self):
        ...
        MenuItemDefaultStyle = self.view.subMenuEditorStyles.Append(
            help="Default Editor Style",
            id=-1,
        ...
        for item in os.listdir(path):
            if item.endswith(".json"):
                ...
                x = self.view.subMenuEditorStyles.Append(
                    help="Editor Style " + _s,
                    id=-1,
                    kind=self.view.wx.ITEM_CHECK,
                    text=_s,
                )
...
class MakerTest(unittest.TestCase):
    ...
    def test_invalidCharsShouldNotCreateProject(self):
        ...
        self.assertEqual(
            self.app.mainView._lastErrorMessage,
            "Please use only Latin characters for project names...",
        )
