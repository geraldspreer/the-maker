...
class AutoComplete(makerController.SuperController):
    ...
    def bindActions(self):
        self.editor.Bind(self.view.wx.stc.EVT_STC_CHARADDED, 
                         self.autoComplete)
    ...
    def autoComplete(self, event):
        ...
        def getArgument(leftOfArg, rightOfArg, searchRange):
            ...
            result = self.editor.FindText(startingPos - searchRange, 
                                          startingPos , 
        ...
        def getCSSIds():
            name = os.path.join(self.model.core.getPathParts(), 
                                self.model.core.getCurrentFileName())
            css = cssTool.listUsedStyleSheetsForFilename(name)
            if not css:
                return None
            listOfIds = []
            for sheet in css:
                for id in cssTool.getIDsFromStyleSheet(os.path.join(self.model.core.getPathParts(), sheet)):
                    listOfIds.append(id)
                   
            return listOfIds
        
        def getCSSClasses():
            name = os.path.join(self.model.core.getPathParts(), self.model.core.getCurrentFileName())
            css = cssTool.listUsedStyleSheetsForFilename(name)
            if not css:
                return None            
            listOfClasses = []
            for sheet in css:
                for id in cssTool.getClassesFromStyleSheet(os.path.join(self.model.core.getPathParts(), sheet)):
        ...
        def autoCompleteHTML():
            ...
            if self.editor.GetTextRange(currentPosition -2 , 
                                        currentPosition -1) == "/":
                # slash found, this is a complete tag
                # eg. <br />
                return
            
            elif self.editor.GetTextRange(currentPosition -2 , 
                                        currentPosition -1) == "-":
            ...
            if openB == -1:
                ...
                if space == -1 or space > currentPosition:
                    ...
                    complete = self.editor.GetTextRange(openB + 1,
                                                         currentPosition - 1)
                else:
    ...
    #                # < a href="#"> fix for leading spaces
        ...
        def getTagList():
            return ['maker_dynamic','body','head','html','span','div id=','style',
                    'meta','link','DOCTYPE','title','em','pre','code','h2','h3',
                    'h1','h6','h4','ins','strong','bdo','dfn','var','samp',
                    'cite','blockquote','acronym','abbr','br />','address','h5',
                    'q','p','del','kbd','a name=','a href=','base','map','object',
                    'param','img src=',
                    'area','dl','ol','dd','li','ul','dt','colgroup','tr','tbody',
                    'caption','tfoot','th','table','td','col','thead','fieldset',
                    'form','textarea','button','label','optgroup','input','legend',
                    'select','option','noscript','script','b','sub','i','big',
                    'tt','hr />','sup','small']
